...
<div class="contentbox">
	...
	<?php echo !empty($row['sn_id']) ? '<a style="color:#fe0000" href="http://m.kuaidi100.com/index_all.html?type='.trim($row['shipping_code']).'&postid='.trim($row['sn_id']).'&callbackurl='.urlencode( SITE_URL.'m/user.php?act=orderlist').'" target="_blank">'.$row['sn_id'].'</a></font>' : '<span class="vieworder" style="color:#fe0000;display:none" id="'.$row['order_id'].'">点击输入物流单</span>';?></td>
	<td>
